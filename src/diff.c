	...
	uint16_t nfiles)
{
	...
	if (nfiles > 2)
		alloc_size += sizeof(git_diff_file) * (nfiles - 2);
	...
	unsigned int nparents, git_tree **parents, const git_index_entry *item)
{
	...
	for (p = 0; error < 0 && p < nparents; ++p) {
		if (git_tree_entry_bypath(&te, parents[p], item->path))
			...
			error = git_oid_equal(git_tree_entry_id(te), &item->oid) ?
				(int)p : -1;
	...
	const git_diff_options *opts)
{
	...
	*diff_ptr = NULL;
	...
	while (!error && !(error = git_iterator_advance(&nitem, iter))) {
		...
		delta = (git_diff_delta_merged *)
			diff_delta__alloc(diff, GIT_DELTA_ADDED, nitem, nparents + 1);
		if (!delta) {
			error = -1;
			break;
		}
		...
		if (matched_parent != GIT_ENOTFOUND) {
			...
			for (p = 0; error < 0 && p < nparents; ++p) {
				if (!p)
					f = (delta->base.status == GIT_DELTA_ADDED) ?
						&delta->base.old_file : &delta->base.new_file;
				else
					f = &delta->extra[p - 1];

				if (git_tree_entry_bypath(&te, parents[p], nitem->path)) {
					giterr_clear();
					f->flags |= GIT_DIFF_FLAG_VALID_OID;
				} else {
					f->mode = git_tree_entry_filemode(te);
					f->size = 0;
					git_oid_cpy(&f->oid, git_tree_entry_id(te));
					git_tree_entry_free(te);
				}
			}

			delta->base.status = GIT_DELTA_MODIFIED;
		}

		notify_res = diff_notify(diff, &delta->base, matched_pathspec);
		if (notify_res) {
			git__free(delta);
			if (notify_res < 0)
				error = GIT_EUSER;
		} else if ((error = git_vector_insert(&diff->deltas, delta)) < 0)
			git__free(delta);
	}
